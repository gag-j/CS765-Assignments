...
class Node():
	...
	def addToBlockchain(self, block):
		...
		self.logs.append(",".join([block.hash, str(self.blocks_received), str("{:.2f}".format(self.env.now)), block.previous_hash])+'\n')
	...
	def receiver(self, data, dtype, sent_by, recursive = False):
		...
		print("{:.2f}: data {} received from {} at {}".format(self.env.now, dtype, sent_by, self.nodeID))
		if dtype == 0 and (data.txnID not in [t.txnID for t in self.txnPool]) and self.blockchain.verifyTxn(sorted(self.txnPool + [data], key = lambda x: x.timestamp))[0]:
			...
			if not recursive:
				...
				blockCheck = 0
			if blockCheck == 0:	
				self.addToBlockchain(data)
				self.txnPool = self.removeTxn(self.txnPool, data.txns)
				print("{:.2f}: data {} broadcasted from {}".format(self.env.now, dtype, self.nodeID))
				self.broadcast(data, dtype, sent_by)

				# check for potential block clashes when mining in progress
				if (not ((self.potentialBlock is None) or (self.interrupt_time > self.start_mine) or (self.start_mine <= self.end_mine)) ) and ((self.potentialBlock.previous_hash == data.previous_hash) or (not self.noTxnClash(data, self.potentialBlock))):
	...
	def generateBlock(self):
		...
		if not (self.interrupt_time >= sleeptime):
			...
			self.addToBlockchain(self.potentialBlock)
